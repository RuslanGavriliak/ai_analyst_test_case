...
- **5 стадий (накопленные AND-гейты):** `Ответ получен → Валидный JSON → Контракт валиден → State верен → Семантическое ядро верно`. Первые три — настоящая воронка выживаемости (технический контракт), последние две — решение и семантическое ядро контекста.
- **Три уровня метрик:**
  ...
  - **Semantic метрики** — точность JSON-полей контекста на задачах с эталоном `ready_for_sql`: отдельно для домена, метрик, источников, сущностей и фильтров.
- **Главный показатель — «Полностью корректный handoff»:** доля задач, прошедших все 5 стадий (строгий AND). Последняя стадия проверяет **семантическое ядро**: `domain_id`, `metric_ids`, `sources`, `entity_ids`. Поле `required_filters` намеренно **не входит** в главный KPI и показывается отдельно как диагностическая метрика. Для `semantic_gap` / `out_of_scope` пустые поля засчитываются как верный ответ. Дополнительно метрика разбивается **по сегментам эталонного `state`**.

#### Какие JSON-поля сравниваются

| Метрика / блок | Какие поля сравниваются | Как сравниваются |
|----------------|--------------------------|------------------|
| **Полностью корректный handoff** | `api_ok`, `json_ok`, `schema_ok`, `state`, затем semantic core: `domain_id`, `metric_ids`, `sources`, `entity_ids` | строгий AND: задача засчитана только если прошла все стадии |
| **Семантическое ядро верно** | `domain_id`, `metric_ids`, `sources`, `entity_ids` | `domain_id` — exact; списки — exact match множеств |
| **Полнота обязат. фильтров** | `required_filters` против обязательных фильтров выбранных метрик из `semantic_layer.json` | recall по полям фильтров; это отдельная диагностическая метрика |
| **Метрики по полям** | `domain_id`, `metric_ids`, `sources`, `entity_ids`, `required_filters` | для `domain_id` — exact; для списков — exact / precision / recall / F1 |
| **State метрики** | `state` | exact accuracy + per-class precision / recall / F1 + confusion matrix |
| **Reason-code метрики** | `reason_codes` | первый код — exact match главной причины; весь список — set F1 |
| **Контракт валиден** | `state`; обязательные списки `metric_ids`, `sources`, `reason_codes`; `domain_id` как строка или `null`; опциональные списки `entity_ids`, `required_filters` | schema/contract check, без сравнения с эталоном по смыслу |

Поэтому возможна ситуация, где **«Семантическое ядро верно» > 0**, а `required_filters` имеет exact/recall около нуля: это разные метрики с разными полями. Фильтры вынесены отдельно намеренно: сравнение `required_filters` шумное, потому что там смешаны машинно-обязательные фильтры semantic layer и свободные текстовые предикаты из вопроса. Из-за этого фильтры лучше использовать как диагностический сигнал, а не смешивать с core semantic identity (`domain_id`, `metric_ids`, `sources`, `entity_ids`) в главном KPI.
...
Вкладка «Различия» (A = `v3`, B = `v2`): 12 значимых различий, 10 — в пользу `v3`. Главный KPI «Полностью корректный handoff» всегда показывается первым.

![Различия v3 и v2 — главный KPI](docs/img/compare_differences_kpi.png)

Для одного прогона обзор показывает ту же логику как воронку из 5 стадий: от технической доступности ответа до строгого semantic core.

![Воронка из 5 стадий — v3](<docs/img/Screenshot 2026-06-19 at 19.17.47.png>)

Ниже — остальные метрики, отсортированные по величине эффекта; зелёный — в пользу `v3`, красный — в пользу `v2`.

![Различия v3 и v2 — остальные метрики](docs/img/compare_differences_cards.png)

#### Диагностика одного прогона (`v3`)

Раздел «State метрики»: классификация `state` — precision/recall/F1 по классам и матрица ошибок.

![State метрики — v3](docs/img/state_metrics.png)

Раздел «Semantic метрики»: корректность JSON-полей контекста на задачах с эталоном `ready_for_sql`. Главный strict KPI проверяет `domain_id`, `metric_ids`, `sources`, `entity_ids`; `required_filters` показан отдельной диагностической строкой/метрикой.

![Semantic метрики — v3](docs/img/semantic_metrics.png)
